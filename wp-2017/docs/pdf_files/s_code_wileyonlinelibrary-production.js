...
s.linkInternalFilters="javascript:,onlinelibrary.wiley.com,olabout.wiley.com,"+ document.domain;

/*var s_account="wileycom-dev"  /* Global RS must be added when go live (wiley-global)
var s=s_gi(s_account)
s.linkInternalFilters="javascript:,onlinelibrary.wiley.com,olabout.wiley.com,"+ document.domain;
*/
...
s.linkDownloadFileTypes="exe,zip,wav,mp3,mp4,mov,mpg,avi,wmv,doc,docx,pdf,xls,xlsx,ppt,pptx,jpg,jpeg,tif,tiff,csv,mol"
s.linkLeaveQueryString=false
s.linkTrackVars="None"
s.linkTrackEvents="None"

/* Form Analysis Config (should be above doPlugins section) */
/* THIS ISNT CURRENTLY USED, IF YOU WOULD LIEK TO USE IT, PLEASE UNCOMMENT
/* s.setupFormAnalysis(); FURTHER BELOW
*/
s.formList="ContactForm,registration"   //MUST GET LIST OF ALL FORMS TO CAPTURE DATA FOR
s.trackFormList=true                    // IF TRUE, ALL FORMS IN formList  WILL BE TRACKED, IF FALSE, ALL FORMS IN formList WILL BE EXCLUDED
...
/* Internal Search Optimization */
    if(s.eVar32){
            s.eVar32=s.eVar32.toLowerCase();
            s.prop32=s.eVar32.toLowerCase();
            var t_search=s.getValOnce(s.eVar32,'ev32',0);
            if(t_search){
                   s.events=s.apl(s.events,"event1",",",2);

                   // merchandising the search term
...
/*Day and time parting props and eVars */
...
s.prop34=s.eVar34=s.getTimeParting('h','-5'); // Set hour
s.prop35=s.eVar35=s.getTimeParting('d','-5'); // Set day
s.prop36=s.eVar36=s.getTimeParting('w','-5'); // Set weekday

/* New v/s Return Visits */
s.prop37=s.eVar37=s.getNewRepeat();

/*Get Visit Number */
s.prop38=s.eVar38=s.getVisitNum();

/* External Campaigns */
s.campaign=(s.getQueryParam('cid','m').length > 0) ? s.getQueryParam('cid','m') : getHashQueryParam('cid','m');     /* CHANGE TO QUERY STRING ID THAT IS USED. eg... (www.....?cid=12345) */
//s.campaign=s.getValOnce(s.getQueryParam('cid','m'),'s_campaign',30);  /* CHANGE TO QUERY STRING ID THAT IS USED. eg... (www.....?cid=12345) */

/* Campaign Medium, Campaign Source & Campaign Unit */
s.eVar28=(s.getQueryParam('cmed').length > 0) ? s.getQueryParam('cmed') : getHashQueryParam('cmed');            // Campaign Medium
s.eVar29=(s.getQueryParam('csource').length > 0) ? s.getQueryParam('csource') : getHashQueryParam('csource');   // Campaign Source
s.eVar30=(s.getQueryParam('cunit').length > 0) ? s.getQueryParam('cunit') : getHashQueryParam('cunit');         // Campaign Unit
//s.eVar28=s.getValOnce(s.getQueryParam('cmed'),'campaign_med',30);         // Campaign Medium
//s.eVar29=s.getValOnce(s.getQueryParam('csource'),'campaign_source',30);   // Campaign Source
//s.eVar30=s.getValOnce(s.getQueryParam('cunit'),'campaign_unit',30);       // Campaign Unit


/* If and When Internal Campaigns are used */
s.eVar41=s.getValOnce(s.getQueryParam('int'),'int_campaign',30);    /* CHANGE TO QUERY STRING ID THAT IS USED. eg... (www.....?int=12345) */

/* DMMS email Query String ID - eg: http://dmmsclick.wiley.ca/click.asp?p=0&m=31802&u=674334 */
s.eVar49=s.getValOnce(s.getQueryParam('p'),'dmms_p',30);    // DMMS Recipient ID
s.eVar50=s.getValOnce(s.getQueryParam('m'),'dmms_m',30);    // DMMS Message ID (also captured in s.campaign)
s.eVar51=s.getValOnce(s.getQueryParam('u'),'dmms_u',30);    // DMMS URL Identifier

if(window.s.eVar40 && s.eVar40!=""){
    s.prop40=s.eVar40;  //Institution Name
  }

/* Copy Props into eVars */
/* Add Sub Section metrics to eVar */
if(window.s.prop5 && s.prop5!=""){
    s.eVar5=s.prop5;    //Site Subsection Level 1
  }

if(window.s.prop6 && s.prop6!=""){
    s.eVar6=s.prop6;    //Site Subsection Level 2
}

if(window.s.prop3 && s.prop3!=""){
    s.eVar3=s.prop3;
}

if(window.s.prop4 && s.prop4!=""){
    s.eVar4=s.prop4;
}
...
/* Force Journal, Book, MRW DOI to lowercase */
if(window.s.prop13 && s.prop13!=""){
...
//s.setupFormAnalysis();   //uncomment this line if you want to enable form analysis plugin to verify the forms listed in s.formList above
